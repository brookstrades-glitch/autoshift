...
import { Submission, Inquiry, FinancingClick } from '@/lib/types';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { Badge } from '@/components/ui/badge';
import AdminTable from './admin-table';
import AdminInquiries from './admin-inquiries';
import AdminFinancing from './admin-financing';
...
interface Props {
  stats: { pending: number; live: number; sold: number; newInquiries: number; financingWeek: number };
  listings: Submission[];
  inquiries: Inquiry[];
  clicks: FinancingClick[];
}

export default function AdminDashboard({ stats, listings, inquiries, clicks }: Props) {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        {[
          ...
          { label: 'Financing (7d)', value: stats.financingWeek },
      ...
      <Tabs defaultValue="listings">
        <TabsList>
          ...
          <TabsTrigger value="financing">
            Financing {stats.financingWeek > 0 && <Badge className="ml-2" variant="secondary">{stats.financingWeek}</Badge>}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="financing" className="mt-4">
          <AdminFinancing clicks={clicks} listings={allListings} />
        </TabsContent>
      </Tabs>
